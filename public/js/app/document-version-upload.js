(function () {
    var modalId = 'upload-document-version-modal';
    var modal = document.getElementById(modalId);
    if (!modal) {
        return;
    }

    var form = document.getElementById('upload-document-version-form');
    var fileInput = document.getElementById('upload-document-version-file');
    var dropzone = document.getElementById('version-native-dropzone');
    var selectedFileLabel = document.getElementById('version-dropzone-selected-file');
    var submitButton = document.getElementById('upload-document-version-submit-btn');
    ...
    function openModal() {
        ...
        window.setTimeout(function () {
            var summary = document.getElementById('upload-version-summary');
            if (summary) {
                summary.focus();
            }
        }, 10);
    ...
    function setSubmitState(isLoading) {
        ...
        submitButton.textContent = isLoading ? 'Uploading...' : 'Upload New Version';
    }
        ...
        fetch(uploadUrl, {
            ...
            .then(function (result) {
                if (!result.ok || result.body.ok !== true) {
                    showToast('error', result.body.message || 'Could not upload new version.');
                    return;
                }

                showToast('success', result.body.message || 'New version uploaded.');
                window.location.href = documentUrl;
            })
            .catch(function () {
                showToast('error', 'Could not upload new version.');
            })
            .finally(function () {
